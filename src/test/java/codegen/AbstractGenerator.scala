  ...
  abstract case class TableDef(val model: m.Table) {
    ...
    */
    /** Definitions to be generated for this table
      *
      * @group Basic customization overrides */
    def definitions = Seq[Def](EntityType, PlainSqlMapper, TableClass, TableValue, TableHelper)
    ...
    /** 动态排序生成器 */
    def TableHelper: TableHelper

    /** 动态排序生成器 */
    trait TableHelperDef extends TermDef {
      def doc = s"dynamicSort for table ${TableValue.name}"
