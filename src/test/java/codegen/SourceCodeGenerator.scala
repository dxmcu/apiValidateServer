...
class SourceCodeGenerator(model: m.Model)
  ...
  class TableDef(model: m.Table) extends super.TableDef(model){
    ...
    type TableHelper =    TableHelperDef
    def  TableHelper=new TableHelper{}
...
object SourceCodeGenerator {
  ...
  def run(profile: String, jdbcDriver: String, url: String, outputDir: String, pkg: String, user: Option[String], password: Option[String], ignoreInvalidDefaults: Boolean, codeGeneratorClass: Option[String], outputToMultipleFiles: Boolean): Unit = {
    ...
    try {
      ...
      val codeGenerator = codeGeneratorClass.getOrElse("slick.com.scalaboot.squirrel.codegen.SourceCodeGenerator")
  ...
  def run(uri: URI, outputDir: Option[String], ignoreInvalidDefaults: Boolean = true, outputToMultipleFiles: Boolean = false): Unit = {
    ...
    val pkg = dc.config.getString("com.scalaboot.squirrel.codegen.package")
    val out = outputDir.getOrElse(dc.config.getStringOr("com.scalaboot.squirrel.codegen.outputDir", "."))
    ...
    args.toList match {
      ...
      case _ => {
        println("""
            ...
            |slick.basic.DatabaseConfig you can set "com.scalaboot.squirrel.codegen.package" and
            |"com.scalaboot.squirrel.codegen.outputDir". The latter can be overridden on the command line.
          """.stripMargin.trim)
        System.exit(1)
      }
    }
