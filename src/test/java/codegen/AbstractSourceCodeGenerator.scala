...
class $name(_tableTag: Tag) extends profile.api.Table[$elementType](_tableTag, ${args.mkString(", ")})$prns {
  ${indent(body.map(_.mkString("\n")).mkString("\n\n"))}
    ...
    trait TableHelperDef extends super.TableHelperDef {
      override def doc = ""

      def dynamicSort = {
        val sortCase = columns.flatMap(c =>
          s"""
             |        case "${c.name}" =>
             |          sort match {
             |            case "asc" => q.sortBy(_.${c.name}.asc)
             |            case "desc" => q.sortBy(_.${c.name}.desc)
             |          }
           """.stripMargin
        ).foldLeft("")((A, B) => A + B)

        s"""def dynamicSort(column: String = "", sort: String = "asc") = {
           |column match {
           |        case "" => q
           |${sortCase}
           |}
      ...
      def dynamicQuery = {
        val queryCase = columns.flatMap(c =>
          s"""case "${c.name}"=>f.${c.name} === B._2.asInstanceOf[${c.rawType}]""" + "\n              "
        ).foldLeft("")((A, B) => A + B)
      ...
      def code = {
        s"""implicit class ${name}Helper(q: Query[Tables.${name}, Tables.${name}#TableElementType, scala.Seq]) {
           |  $dynamicQuery
           |  $dynamicSort
           |}""".stripMargin
      }
