...
trait ParameterTable {
  ...
  case class ParameterRow(projectName: String, bindType: String, requestMethod: String, requestMapping: String, paramName: String, paramType: String, format: Option[String] = None, regular: Option[String] = None, enable: Boolean = false, createAt: java.sql.Timestamp, updateAt: java.sql.Timestamp)
  implicit def GetResultParameterRow(implicit e0: GR[String], e1: GR[Option[String]], e2: GR[Boolean], e3: GR[java.sql.Timestamp]): GR[ParameterRow] = GR{
    prs => import prs._
    ParameterRow.tupled((<<[String], <<[String], <<[String], <<[String], <<[String], <<[String], <<?[String], <<?[String], <<[Boolean], <<[java.sql.Timestamp], <<[java.sql.Timestamp]))
  }
  class Parameter(_tableTag: Tag) extends profile.api.Table[ParameterRow](_tableTag, Some("api_validate"), "parameter") {
    def * = (projectName, bindType, requestMethod, requestMapping, paramName, paramType, format, regular, enable, createAt, updateAt) <> (ParameterRow.tupled, ParameterRow.unapply)
    def ? = ((Rep.Some(projectName), Rep.Some(bindType), Rep.Some(requestMethod), Rep.Some(requestMapping), Rep.Some(paramName), Rep.Some(paramType), format, regular, Rep.Some(enable), Rep.Some(createAt), Rep.Some(updateAt))).shaped.<>({r=>import r._; _1.map(_=> ParameterRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get, _7, _8, _9.get, _10.get, _11.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))

    val projectName: Rep[String] = column[String]("project_name", O.PrimaryKey, O.Length(255,varying=true))
