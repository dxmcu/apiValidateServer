...
trait BeanTable {
  ...
  case class BeanRow(beanName: String, projectName: String, beanType: String, createAt: java.sql.Timestamp, updateAt: java.sql.Timestamp)
  implicit def GetResultBeanRow(implicit e0: GR[String], e1: GR[java.sql.Timestamp]): GR[BeanRow] = GR{
    prs => import prs._
    BeanRow.tupled((<<[String], <<[String], <<[String], <<[java.sql.Timestamp], <<[java.sql.Timestamp]))
  }
  class Bean(_tableTag: Tag) extends profile.api.Table[BeanRow](_tableTag, Some("api_validate"), "bean") {
    def * = (beanName, projectName, beanType, createAt, updateAt) <> (BeanRow.tupled, BeanRow.unapply)
    def ? = ((Rep.Some(beanName), Rep.Some(projectName), Rep.Some(beanType), Rep.Some(createAt), Rep.Some(updateAt))).shaped.<>({r=>import r._; _1.map(_=> BeanRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
